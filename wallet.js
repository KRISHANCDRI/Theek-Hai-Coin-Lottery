...
// --- SOL/USD fetch (Jupiter API + Coingecko fallback) ---
async function getSolUsdPrice() {
    // Try Jupiter first
    try {
        let res = await fetch('https://price.jup.ag/v4/price?ids=SOL');
        let data = await res.json();
    ...
    // (Optional fallback: static value)
    // return 150;
    return null;
}
...
buyBtn.addEventListener('click', async () => {
    ...
    // --- PUMP.FUN REDIRECT LOGIC (balance 0 ya kam hai) ---
    ...
    // If user has enough TH COIN: SUBMIT TICKET (TOKEN TRANSFER)
    try {
        ...
        // IMMEDIATELY REDIRECT (no need to wait for confirmTransaction)
